...
from plots import make_data_plots
...
def clean_data(generated, load, save_raw):
    ...
    generated_clean = (
        generated.groupby(["HourlyTime", "Country"]).agg({"UnitName": "first", "quantity": "sum"}).reset_index()
    )
    ...
    load_clean = load.groupby(["HourlyTime", "Country"]).agg({"UnitName": "first", "Load": "sum"}).reset_index()
...
def preprocess_data(df):
    ...
    df_pivoted.columns = [
        "{}_{}{}".format(col[0], col[1], "" if len(col) < 3 else col[2]) for col in df_pivoted.columns
    ]
    new_columns = []
    ...
    # Añadir columan "Result" y añadir valores random entre 1 y 10
    df_pivoted["Result"] = random.choices(range(1, 11), k=len(df_pivoted))
...
def validate_data(df):
    ...
    if df.isnull().values.any():
        logger.error("There are NaNs in the data")
...
def parse_arguments():
    ...
    parser.add_argument(
        "--input_file", type=str, default="data/raw_data.csv", help="Path to the raw data file to process"
    )
    parser.add_argument(
        "--output_file", type=str, default="data/processed_data.csv", help="Path to save the processed data"
    )
    parser.add_argument("--folder_path", type=str, default="data/", help="Path to save the processed data")
    return parser.parse_args()


def main(input_file=None, output_file=None, data_folder=None, save_raw=True):
